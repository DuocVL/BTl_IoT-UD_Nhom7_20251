...
const char* topic_telemetry = "smart-home/controller-01/telemetry";
...
void publishStatus(DeviceState &dev) {
  ...
  snprintf(topicBuffer, sizeof(topicBuffer), "smart-home/controller-01/%s/status", dev.id.c_str());
...
void reconnect() {
  while (!client.connected()) {
    ...
    if (client.connect(clientId.c_str(), mqtt_user, mqtt_pass)) {
      client.subscribe("smart-home/controller-01/cmd");
